...
def segmentStableNotesRegions(inputFile = '../../sounds/sax-phrase-short.wav', stdThsld=10, minNoteDur=0.1, 
    ...
    print 'f0Cents = ', f0Cents
    sdArr = np.array([])
    for i in range(winStable-1, len(f0Cents)+1):
        subf0Cents = f0Cents[i-winStable-1:i+1]
        sdArr = np.append(sdArr, np.std(subf0Cents))
    
    print 'len(f0Cents), len(sdArr)', len(f0Cents), len(sdArr)
    print 'sdArr', sdArr

    new_f0Cents = f0Cents[winStable:]
    new_f0 = f0[winStable:]

    #3. apply threshold on standard deviation values to find indexes of the stable points in melody
    indx = np.zeros(len(sdArr))

    for i in range(0,len(sdArr)):
        if sdArr[i] < stdThsld:
            indx[i] = 1
    print 'indx, len(indx), len(new_f0Cents)', indx, len(indx), len(new_f0Cents)
    ...
    segment = np.array([1,1])
    print 'segment', segment

    for i in range(0, len(indx)-1):
        if indx[i] == 0 and indx[i+1] == 1:
            j = i+1

            while indx[j] == 1 and j != len(indx)-1:
                j = j+1

            if (j-i)>= minNoteDur * fs / H:
                segment = np.vstack([segment, [i+1, j]])
                print 'check'
