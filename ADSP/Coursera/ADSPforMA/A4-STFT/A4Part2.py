...
def computeSNR(inputFile, window, M, N, H):
    ...
    print 'lenX and N', len(X), N

    Noise = X-x
    print 'Noise = ', Noise

    cutX = X[M:len(X)-M]
    cutNoise = Noise[M:len(X)-M]


    absX = (abs(X))**2
    Ex = np.sum(absX)

    absNoise = (abs(Noise))**2
    Enoise = np.sum(absNoise)

    absCutX = (abs(cutX))**2
    ECutX = np.sum(absCutX)

    absCutNoise = (abs(cutNoise))**2
    ECutNoise = np.sum(absCutNoise)

    SNR1 = 10*np.log10(Ex/Enoise)
    SNR2 = 10*np.log10(ECutX/ECutNoise)
...
computeSNR('piano.wav', 'blackman', 513, 2048, 128)
