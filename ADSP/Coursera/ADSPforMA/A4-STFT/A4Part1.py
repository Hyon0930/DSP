...
def extractMainLobe(window, M):
    ...
    print 'cent_mX = ', cent_mX

    firstHalf = cent_mX[:hN]
    print 'firstHalf = ', firstHalf
    secondHalf = cent_mX[hN:]
    print 'secondHalf = ', secondHalf

    mainLobe = np.array([])
    chkmainLobe = np.array([])

    for i in range(0, len(secondHalf)):
        print secondHalf[i+1]
        if secondHalf[i] > secondHalf[i+1]:
            chkmainLobe = np.append(chkmainLobe, secondHalf[i+1])
            print i
        else:
            break
    
    firstHalf = np.flipud(chkmainLobe)

    mainLobe = np.append(firstHalf, secondHalf[0])
    mainLobe = np.append(mainLobe, chkmainLobe)

    print 'mainLobe = ', len(mainLobe), mainLobe
