...
def computeEngEnv(inputFile, window, M, N, H):
    ...
    #get index for the freqency band

    k0 = int(np.floor(float(N)/fs*0)+1);
    k1 = int(np.floor(float(N)/fs*f1)+1);
    k2 = int(np.floor(float(N)/fs*f2)+1);
    ...
    env_low = [np.sum((10**(mX[frame][k0:k1]/20))**2) for frame in np.arange(len(mX))]
    env_high = [np.sum((10**(mX[frame][k1:k2]/20))**2) for frame in np.arange(len(mX))]

    # env_low[env_low<frame] = eps
    # env_high[env_high<frame] = eps
    for frame in np.arange(len(mX)):
        if env_low[frame] < eps:
            env_low[frame] = eps
        if env_high[frame] < eps:
            env_high[frame] = eps

    env_low = 10*np.log10(env_low)
    env_high = 10*np.log10(env_high)

    engEnv = [env_low, env_high]

    return np.transpose(engEnv)


computeEngEnv('piano.wav', 'blackman', 513, 1024, 128)
